...
int main(int argc, char** argv) {
	...
	int count_all, count_sta, count_squ, count_lie;
	int hog_dim;

	//CoarseDetectorの取り込み
	if ((CD = svm_load_model("C:/model_file/pre_model/CD_pre.model")) == 0)exit(1);
	if ((Stand = svm_load_model("C:/model_file/pre_model/Stand.model")) == 0)exit(1);
	if ((Squat = svm_load_model("C:/model_file/pre_model/Squat.model")) == 0)exit(1);
	if ((Lie = svm_load_model("C:/model_file/pre_model/Lie.model")) == 0)exit(1);
	...
	while (fgets(test_name, 256, test_data) != NULL) {
		...
		for (int i = 0; name.length() - 1; i++) {
			test[i] = test_name[i];
			test[i + 1] = '\0';
		}
		
		char result_data[2][1024] = {
			"C:/Users/k1410159/source/repos/Human_Detection_for_Only_One_Pose/Human_Detection_for_Only_One_Pose/result_data/CD/",
			"C:/Users/k1410159/source/repos/Human_Detection_for_Only_One_Pose/Human_Detection_for_Only_One_Pose/result_data/all/",
		};

		char result_binary_all[2][1024] = {
			"C:/Users/k1410159/source/repos/Human_Detection_for_Only_One_Pose/Human_Detection_for_Only_One_Pose/result_binary/CD/",
			"C:/Users/k1410159/source/repos/Human_Detection_for_Only_One_Pose/Human_Detection_for_Only_One_Pose/result_binary/all/",
		};
...
//		cv::Mat ans_img_all = cv::imread("Sun_Nov_26_11_21_46_84.bmp", 1);	//検出する画像
		...
		cv::Mat check_img = ans_img_all.clone();

		cv::imshow("", ans_img_all);
		cvWaitKey(0);
...
//		file_num++;
		...
		Detect_Place det_CD[200];
		Detect_Place det_ST[200];
		Detect_Place det_SQ[200];
		Detect_Place det_LI[200];
		...
		for (int img_size = 0; normalize_num[img_size] != -1; img_size++) {
			...
			cvtColor(ans_img_all, img, CV_RGB2GRAY);
		...
		//領域の統一
		int t_num = 0;
		for (int n = 0; det_CD[n].C_yudo != 0; n++) {
			if (det_CD[n].territory_num == -1) {
				t_num++;
				det_CD[n].territory_num = t_num;
			}
...
//			fprintf_s(result_text, " , %d, %d, %d, %d", detect[final_num].C_x, detect[final_num].C_y, detect[final_num].C_width, detect[final_num].C_height);
		...
		//領域の統一・立位
		t_num = 0;
		for (int n = 0; det_ST[n].C_yudo != 0; n++) {
			if (det_ST[n].territory_num == -1) {
				t_num++;
				det_ST[n].territory_num = t_num;
			}
			for (int m = n + 1; det_ST[m].C_yudo != 0; m++) {
				if ((det_ST[n].C_x + det_ST[n].C_width/2) - 100 <= (det_ST[m].C_x + det_ST[m].C_width/2)
					&&
					(det_ST[m].C_x + det_ST[m].C_width/2) <= (det_ST[n].C_x + det_ST[n].C_width/2) + 100
					&&
					(det_ST[n].C_y + det_ST[n].C_height/2) - 100 <= (det_ST[m].C_y + det_ST[m].C_height/2)
					&&
					(det_ST[m].C_y + det_ST[m].C_height/2) <= (det_ST[n].C_y + det_ST[n].C_height/2) + 100) {
		...
		for (int i = 1; i <= t_num; i++) {
			int final_num = 0;
			float cyudo = 0;
			for (int k = 0; det_ST[k].C_yudo != 0; k++) {

				if (det_ST[k].territory_num == i && det_ST[k].C_yudo > cyudo) {
					final_num = k;
					cyudo = det_ST[k].C_yudo;

				}
			}

			//矩形表示
			ans_img_all = draw_rectangle(ans_img_all, det_ST[final_num].C_x, det_ST[final_num].C_y, det_ST[final_num].C_width, det_ST[final_num].C_height, 0, 255, 0);

			//結果をテキストファイルに保存
			//			fprintf_s(result_text, " , %d, %d, %d, %d", detect[final_num].C_x, detect[final_num].C_y, detect[final_num].C_width, detect[final_num].C_height);

			//			for (int n = detect[final_num].C_y; n < detect[final_num].C_y + detect[final_num].C_height ; n++) {
			//				for (int m = detect[final_num].C_x; n < detect[final_num].C_x + detect[final_num].C_width ; m++) {
		...
		//領域の統一・座位
		t_num = 0;
		for (int n = 0; det_SQ[n].C_yudo != 0; n++) {
			if (det_SQ[n].territory_num == -1) {
				t_num++;
				det_SQ[n].territory_num = t_num;
			}
			for (int m = n + 1; det_SQ[m].C_yudo != 0; m++) {
				if ((det_SQ[n].C_x + det_SQ[n].C_width/2) - 100 <= (det_SQ[m].C_x + det_SQ[m].C_width/2)
					&&
					(det_SQ[m].C_x + det_SQ[m].C_width/2) <= (det_SQ[n].C_x + det_SQ[n].C_width/2) + 100
					&&
					(det_SQ[n].C_y + det_SQ[n].C_height/2) - 100 <= (det_SQ[m].C_y + det_SQ[m].C_height/2)
					&&
					(det_SQ[m].C_y + det_SQ[m].C_height/2) <= (det_SQ[n].C_y + det_SQ[n].C_height/2) + 100) {

					det_SQ[m].territory_num = det_SQ[n].territory_num;
				}
			}
		...
		for (int i = 1; i <= t_num; i++) {
			int final_num = 0;
			float cyudo = 0;
			for (int k = 0; det_SQ[k].C_yudo != 0; k++) {

				if (det_SQ[k].territory_num == i && det_SQ[k].C_yudo > cyudo) {
					final_num = k;
					cyudo = det_SQ[k].C_yudo;

				}
			}

			//矩形表示
			ans_img_all = draw_rectangle(ans_img_all, det_SQ[final_num].C_x, det_SQ[final_num].C_y, det_SQ[final_num].C_width, det_SQ[final_num].C_height, 255, 0, 0);
		...
		//領域の統一・臥位
		t_num = 0;
		for (int n = 0; det_LI[n].C_yudo != 0; n++) {
			if (det_LI[n].territory_num == -1) {
				t_num++;
				det_LI[n].territory_num = t_num;
			}
			for (int m = n + 1; det_LI[m].C_yudo != 0; m++) {
				if ((det_LI[n].C_x + det_LI[n].C_width/2) - 100 <= (det_LI[m].C_x + det_LI[m].C_width/2)
					&&
					(det_LI[m].C_x + det_LI[m].C_width/2) <= (det_LI[n].C_x + det_LI[n].C_width/2) + 100
					&&
					(det_LI[n].C_y + det_LI[n].C_height/2) - 100 <= (det_LI[m].C_y + det_LI[m].C_height/2)
					&&
					(det_LI[m].C_y + det_LI[m].C_height/2) <= (det_LI[n].C_y + det_LI[n].C_height/2) + 100) {

					det_LI[m].territory_num = det_LI[n].territory_num;
				}
			}
		...
		for (int i = 1; i <= t_num; i++) {
			int final_num = 0;
			float cyudo = 0;
			for (int k = 0; det_LI[k].C_yudo != 0; k++) {

				if (det_LI[k].territory_num == i && det_LI[k].C_yudo > cyudo) {
					final_num = k;
					cyudo = det_LI[k].C_yudo;

				}
			}

			//矩形表示
			ans_img_all = draw_rectangle(ans_img_all, det_LI[final_num].C_x, det_LI[final_num].C_y, det_LI[final_num].C_width, det_LI[final_num].C_height, 0, 0, 255);
...
//		cv::imwrite(new_result_name, ans_img_all);
